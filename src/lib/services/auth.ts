...
export async function dryCreateUser(
  username: string,
  email: string,
  password: string
) {
  const userByUsername = await pb
    .collection("users")
...
export function validateStageOneSignUpData(
  data: StageOneSignUpDetails
): boolean {
  const signUpData = z.object({
    ...
    confirmPassword: z
      .string()
      .refine((password) => password === data.password && password.length > 0, {
